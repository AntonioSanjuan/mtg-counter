...
describe('GameRestart', () => {
  let gameRestartStore: any;
  let history: any;

  beforeEach(() => {
    gameRestartStore = createTestStore();
  ...
  it('should create', () => {
    const { container } = render(
      <Provider store={gameRestartStore}>
  ...
  it('saveButton click should open alert GameSave', async () => {
    ...
    const saveButton = screen.getByLabelText('GameRestart_OkButton');
    ...
    expect(mock_useAlert.mock().openAlert).toHaveBeenCalledWith(DynamicAlertTypes.GameSave)
  });

  it('restartButton click should request restartGame and close alert', async () => {
    render(
      <Provider store={gameRestartStore}>
        <Router location={history.location} navigator={history}>
          <GameRestart />
    ...
    expect(mock_useGameManagement.mock().restartGame).not.toHaveBeenCalled()
    expect(mock_useAlert.mock().closeAlert).not.toHaveBeenCalled()

    const restartButton = screen.getByLabelText('GameRestart_CancelButton');
